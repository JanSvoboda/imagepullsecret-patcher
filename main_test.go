...
func helperCreateOpaqueSecret(k8s *k8sClient) error {
	_, err := k8s.clientset.CoreV1().Secrets(v1.NamespaceDefault).Create(context.Background(),
		&v1.Secret{
			ObjectMeta: metav1.ObjectMeta{
...
func helperCreateServiceAccountWithoutImagePullSecret(serviceAccountName string) step {
	return func(k8s *k8sClient) error {
		_, err := k8s.clientset.CoreV1().ServiceAccounts(v1.NamespaceDefault).Create(context.Background(),
			&v1.ServiceAccount{
				ObjectMeta: metav1.ObjectMeta{
